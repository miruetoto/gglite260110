---
title: "gglite: A Lightweight and Intuitive R Package for Extending Graphic Grammar"
...
- *실험결과돌아간것 나한테 리포트해줘, (`xxxx`폴더에 결과있음) 최적값이 뭐야?* 
- *하나돌리는 얼마나 걸렸어?* 
- *gamma=0.5에서 값만 알려줄래?*

`예제5` -- LaTeX 표 삽입 

- *gamma $<$ 0.5인 결과만 선택해서 `paper.tex`의 섹션3의 적당한 위치에 표를  정리해서 넣어줘. 표는 (`cmd+shift+4` $\to$ `cmd+v` 로 그림을 붙여넣으며) 그림과 비슷한 양식으로 만들어줘.* 
...
- 이런 그림도 클로드코드로 쉽게 그릴 수 있고

![ . ](figs/복잡한그림1.png){height="400px"}

#### 복잡한그림2

- 이런 그림을 그리는 코드도 쉽게 구할 수 있음. 

![ . ](figs/복잡한그림2.png){height="400px"}

#### 간단한그림

- 그런데 이런 그림을 그릴때도 코드를 물어봐야 할까? 
...
- **과거**: ggplot2의 단순 wrapper에 불과하다고 생각
- **현재**: 에이전틱 코딩 시대에 "간결한 문법"의 가치 재발견
